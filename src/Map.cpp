...
//Getting the image
Map::Map() :
    arrowSprite(SDL_LoadBMP("../Project 2 [CSCI 115]/Sprite/Arrow.bmp")),
    wallSprite(SDL_LoadBMP("../Project 2 [CSCI 115]/Sprite/Wall.bmp")),
    floorSprite(SDL_LoadBMP("../Project 2 [CSCI 115]/Sprite/Floor.bmp")),
    chestSprite(SDL_LoadBMP("../Project 2 [CSCI 115]/Sprite/Chest.bmp"))
{
...
void Map::mapText()
{
    ...
    if(mapWall.is_open())
    {
        while(getline(mapWall, line))
        {
            ...
            {
                if((mapMaze.back())[i] == 'X')
                    wallPosition.push_back({i * 24, (mapMaze.size() - 1) * 24, 0, 0});
                else
                {
                    if((mapMaze.back())[i] == 'P')
                        Player::playerPosition = {i * 24, (mapMaze.size() - 1) * 24, 0, 0};
                    else if((mapMaze.back())[i] == 'E')
                        Enemy::enemyPosition.push_back({i * 24, (mapMaze.size() - 1) * 24, 0, 0});
                    else if((mapMaze.back())[i] == 'A')
                        arrowPosition.push_back({i * 24, (mapMaze.size() - 1) * 24, 0, 0});
                    else if((mapMaze.back())[i] == 'C')
                        chestPostion = {i * 24, (mapMaze.size() - 1) * 24, 0, 0};

                    floorPosition.push_back({i * 24, (mapMaze.size() - 1) * 24, 0, 0});
                }
            }
        }
        mapWall.close();
